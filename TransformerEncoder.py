...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, embedding_weight,vocab_size, seq_len, d_model=512, n_layers=6, n_heads=8, p_drop=0.1, d_ff=2048, pad_id=0,fix_embedding = True):
        ...
        if type(embedding_weight) != bool: self.embedding.weight = nn.Parameter(embedding_weight)# 如果有训练好的emb输入   也可以写成 self.emb = nn.from_pretrained(embedding_weight)
    ...
    def forward(self, inputs):
        ...
        position_pad_mask = inputs.eq(self.pad_id)  # position_pad_mask:pad过的位置为1，其它位置为0
